...
Banana.prototype.launch = function(angleDeg, power){
	...
	// flag to know if the banana flew away from the shooter to check a self-destruction
	this.isFlyAway = false;
};

/**
* Checks if banana hit target gorilla.
*/
...
/**
* Checks a self-destruction if banana hit shooter gorilla.
*/
Banana.prototype.isHitShooter = function(gorilla){
	var boundsA = this.getBounds();
	var boundsB = gorilla.getBounds();

	var isCollide = Phaser.Rectangle.intersects(boundsA, boundsB);
	
	if (!this.isFlyAway && !isCollide) this.isFlyAway = true;
	
	return (this.isFlyAway & isCollide);
};
